...
function send_email(array $data) {
    ...
    $name = $data['name'];
    $subject = 'Activation to Red-shouldered Hawk';
    $email_from = 'jrpepp2014@jrpepp.com';
    if (filter_input(INPUT_SERVER, 'SERVER_NAME', FILTER_SANITIZE_URL) == "localhost") {
        $comments = 'Here is you confirmation link: http://localhost:8888/phpdigitalpaper/activate.php?confirmation=' . $data['confirmation'];
    } else {
        $comments = 'Here is you confirmation link: https://www.redshoulderedhawkblog.com/activate.php?confirmation=' . $data['confirmation'];
    }
    $message->setTo([
    ...
    $message->setFrom($email_from, 'PHP Digital Paper'); // From and Name:
...
function verifyConfirmation($email, $confirmation, $pdo = NULL) {
    $query = 'SELECT id, email, confirmation FROM mysimpleregistration WHERE email=:email';
...
function upgradeAccount($id, $pdo = NULL) {
    $sql = "UPDATE mysimpleregistration SET confirmation='confirmed', security='member' WHERE id=:id";
